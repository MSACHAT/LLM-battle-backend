package com.example.llm_rating.config.sec;
 ...
 * @author luozhan
 * @date 2019-05
// @see https://github.com/luo-zhan/LambdaExceptionThrower
 */
...
public class LambdaExceptionUtil {
     ...
     * Exception 'XXX' is never thrown in the corresponding try
     * block（在try语句体中永远不会抛出相应异常）
     */
    ...
    public static <T, U, R, E extends Exception> BiFunction<T, U, R> wrapBiFunction(
            BiFunctionWithExceptions<T, U, R, E> biFunction) throws E {
        return (t, u) -> {
            try {
    ...
    public static <T, U, E extends Exception> BiPredicate<T, U> wrapBiPredicate(
            BiPredicateWithExceptions<T, U, E> predicate) throws E {
        return (t, u) -> {
            try {
